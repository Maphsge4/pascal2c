...
void getStatementList(Type *now, vector<_Statement *> &_statementList) {
    ...
    int loc = int(now->children.size() - 1);
    _Statement *statement = getStatement(now->children[loc]); // 这个地方，得到的是"特定语句"类型的指针，但是强制转成了Statement类型
    if (statement != NULL)
...
_FunctionDefinition *getSubprogramDefinition(Type *now) {
    ...
    getSubprogramHead(now->children[0], _functionDefinition->functionID, _functionDefinition->formalParaList,
                      _functionDefinition->type);
    getSubprogramBody(now->children[2], _functionDefinition->constList, _functionDefinition->variantList,
...
_Program *getProgram(Type *now) {
    if (now->token != "programstruct") { // 根节点的名字必须是programstruct
