...
void _SymbolTable::addPara(string id, int lineNumber, string type, bool e) {
...
void
_SymbolTable::addArray(string id, int lineNumber, string type, int amount, vector<pair<int, int> > arrayRangeList) {
    _SymbolRecord *tmpRecord = new _SymbolRecord;
...
void _SymbolTable::addVoidPara(string id, int lineNumber) {
    if (this->idToLoc.count(id)) { // 符号表中有同名的
        for (int i = 0; i < idCount[id]; i++)
            id = "_" + id; // 把原来符号表里的都加上下划线予以区分
    }
    _SymbolRecord *tmpRecord = new _SymbolRecord;
    tmpRecord->setVoidPara(id, lineNumber);
    this->recordList.push_back(tmpRecord);
    this->idToLoc[id] = int(recordList.size() - 1); // 名字和vector下标对应起来
    if (this->idCount.count(id))
        this->idCount[id]++;
    else
        this->idCount[id] = 1;
...
void
_SymbolRecord::setArray(string id, int lineNumber, string type, int amount, vector<pair<int, int> > arrayRangeList) {
    flag = "array";
    this->id = id;
    this->lineNumber = lineNumber;
    this->type = type;
...
string _SymbolRecord::findXthFormalParaType(int X) {
    if ((flag == "function" || flag == "procedure") && X >= 1 && X < amount + 1)
        ...
        cout
                << "[_SymbolRecord::findXthFormalParaType] record is not a function or procedure, or formal parameter index out of range"
                << endl;
        return "";
    }
}
...
bool _SymbolRecord::isXthFormalParaRefered(int X) {
    if ((flag == "function" || flag == "procedure") && X >= 1 && X < amount + 1)
        ...
        cout
                << "[_SymbolRecord::isXthFormalParaRefered] record is not a function or procedure, or formal parameter index out of range"
                << endl;
        return "";
    }
}
