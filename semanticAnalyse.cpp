...
extern int
inputExpression(_Expression *expressionNode, string &expression, int mode = 0, bool isReferedActualPara = false);//获取表达式
extern void inputVariantRef(_VariantReference *variantRefNode, string &variantRef, int mode = 0,
                            bool isReferedActualPara = false);//获取变量引用
...
void SemanticAnalyseRestProgram(_SubProgram *subprogram);//对主程序剩余部分进行语义分析
...
//添加重定义错误信息
void addDuplicateDefinitionErrorInformation(string preId, int preLineNumber, string preFlag, string preType,
                                            int curLineNumber);//获得重复定义的语义错误信息
//添加未定义错误信息
...
void SemanticAnalyse(_Program *ASTRoot) {
    currentSymbolTable = mainSymbolTable = new _SymbolTable("main");//创建主符号表，指定为主符号表后，会自动加入read, write等库函数
    ...
    if (variantReference->flag == 0) {//如果是非数组
        if (record->flag == "(sub)program name") {
            if (record->subprogramType == "procedure") {//这个包含了查到主程序名的情况
                ...
                addGeneralErrorInformation("[Invalid reference] <Line " + itos(variantReference->variantId.second) +
                                           "> Procedure name \"" + record->id + "\" can't be referenced");
            ...
            if (record->amount != 0) {//如果形参个数不为0 checked 递归调用
                addNumberErrorInformation(variantReference->variantId.first, variantReference->variantId.second, 0,
                                          record->amount, "function");
                return variantReference->variantType = record->type;
            }
            //如果形参个数为0
        ...
        if (record->flag == "function") {//如果是函数 则一定是在主符号表中查到的
            ...
            if (variantReference->locFlag == -1) {//如果是左值 checked
                addGeneralErrorInformation("[Invalid reference!] <Line " + itos(variantReference->variantId.second) +
                                           "> function name \"" + record->id + "\" can't be referenced as l-value.");
                return variantReference->variantType = "error";
            }
            //如果是右值
        ...
        if (!(record->flag == "value parameter" || record->flag == "var parameter" ||
              record->flag == "normal variant" || record->flag == "constant")) {
            addGeneralErrorInformation(
                    "[Invalid reference!] <Line " + itos(variantReference->variantId.second) + "> \"" +
                    variantReference->variantId.first + "\" is a " + record->flag + ", it can't be referenced.");
            return variantReference->variantType = "error";
        }
        ...
        if (record->flag != "array") {//如果符号表中记录的不是数组 checked
            addPreFlagErrorInformation(variantReference->variantId.first, variantReference->variantId.second, "array",
        ...
        if (variantReference->expressionList.size() != record->amount) {//如果引用时的下标维数和符号表所存不一致
            addNumberErrorInformation(variantReference->variantId.first, variantReference->variantId.second,
                                      int(variantReference->expressionList.size()), record->amount, "array");
        ...
        for (int i = 0; i < variantReference->expressionList.size(); i++) {
            ...
            if (type != "integer") {
                addExpressionTypeErrorInformation(variantReference->expressionList[i], type, "integer",
                                                  itos(i + 1) + "th index of array \"" +
                                                  variantReference->variantId.first + "\"");
                variantReference->variantType = "error";
            }
            //检查越界 checked
            if (variantReference->expressionList[i]->totalIntValueValid) {
                if (!record->checkArrayXthIndexRange(i, variantReference->expressionList[i]->totalIntValue)) {
                    addArrayRangeOutOfBoundErrorInformation(variantReference->expressionList[i],
                                                            variantReference->variantId.first, i,
...
void SemanticAnalyseStatement(_Statement *statement) {
    // 形参用Statement类型为了统一格式，进来的时候都变成statement，进来以后再根据type域转换
    if (statement == NULL) {
        cout << "[SemanticAnalyseStatement] pointer of _Statement is null" << endl;
        return;
    }

    if (statement->type == "compound") {  // 第一轮分析时，本身就是compound类型强制转换过来的
        ...
        SemanticAnalyseStatement(Statement->_do);//对循环体语句进行语义分析
    }
        ...
        //如果无法作为循环变量 checked
        if (!(record->flag == "value parameter" || record->flag == "var parameter" ||
              record->flag == "normal variant")) {//如果当前符号种类不可能作为循环变量
            addPreFlagErrorInformation(forStatement->id.first, forStatement->id.second,
                                       "value parameter, var parameter or normal variant", record->lineNumber,
                                       record->flag);
            return;
        }
        ...
        if (assignStatement->variantReference->kind == "constant") {
            //左值不能为常量 checked
            addGeneralErrorInformation(
                    "[Constant as l-value error!] <Line" + itos(assignStatement->variantReference->variantId.second) +
                    "> Costant \"" + assignStatement->variantReference->variantId.first +
        ...
        if (assignStatement->variantReference->kind == "function return reference") {//如果是返回值语句
            ...
            if (assignStatement->variantReference->variantType != rightType &&
                !(assignStatement->variantReference->variantType == "real" and rightType == "integer")) {
                //checked
                addGeneralErrorInformation(
                        "[Return type of funciton mismatch!] <Line " + itos(assignStatement->expression->lineNumber) +
...
//                    procedureCall->statementType = "error";
            ...
            if (procedureCall->actualParaList.size() != 1) {//如果实参个数不为1
                if (procedureCall->actualParaList.size() == 0) //checked
                    addGeneralErrorInformation(
                            "[Return value missing!] <Line " + itos(procedureCall->procedureId.second) +
                            "> Number of return value of function must be 1, that is, exit must have 1 actual parameters.");
                else //checked
                    addGeneralErrorInformation(
                            "[Return value redundancy!] <Line " + itos(procedureCall->procedureId.second) +
                            "> Number of return value of function must be 1, that is, exit must have 1 actual parameters.");
            ...
            if (currentSymbolTable->recordList[0]->type != returnType &&
                !(currentSymbolTable->recordList[0]->type == "real" && returnType == "integer")) {
                //checked
                addGeneralErrorInformation("[Return type of funciton mismatch!] <Line " +
                                           itos(procedureCall->actualParaList[0]->lineNumber) +
        ...
        if (record->id == "read") {//参数只能是变量或数组元素，不能是常量、表达式等
            for (int i = 0; i < procedureCall->actualParaList.size(); i++) {
                ...
                //checked
                if (!(procedureCall->actualParaList[i]->type == "var" &&
                      (procedureCall->actualParaList[i]->variantReference->kind == "var" ||
                       procedureCall->actualParaList[i]->variantReference->kind == "array")))
                    addactualParameterOfReadErrorInformation(procedureCall->actualParaList[i]->lineNumber, record->id,
        ...
        for (int i = 0; i < procedureCall->actualParaList.size(); i++) {//检查actualParaList各表达式的类型，检查实参和形参的类型一致性
            ...
            if (isRefered && !(procedureCall->actualParaList[i]->type == "var" &&
                               (procedureCall->actualParaList[i]->variantReference->kind == "var" ||
                                procedureCall->actualParaList[i]->variantReference->kind == "array"))) {
                //该表达式不能作为引用形参对应的实参 checked
                addGeneralErrorInformation("[Referenced actual parameter error!] <Line " +
                                           itos(procedureCall->actualParaList[i]->lineNumber) + "> The " + itos(i + 1) +
                                           "th actual parameter expression should be a normal variable、value parameter、referenced parameter or array element.");
                continue;
            }
...
void SemanticAnalyseSubprogramDefinition(_FunctionDefinition *functionDefinition) {
    ...
    string subprogramType;// 看有没有返回值，辨别是函数还是过程
    ...
    currentSymbolTable->addProgramName(functionDefinition->functionID.first, functionDefinition->functionID.second,
                                       ...
                                       functionDefinition->type.first);

    //根据type是否为NULL，分为addProcedure()和addFunction()，添加到主程序表中
    if (functionDefinition->type.first == "")//如果是过程
        mainSymbolTable->addProcedure(functionDefinition->functionID.first, functionDefinition->functionID.second,
                                      int(functionDefinition->formalParaList.size()), currentSymbolTable);
    else//如果是函数
        mainSymbolTable->addFunction(functionDefinition->functionID.first, functionDefinition->functionID.second,
                                     functionDefinition->type.first, int(functionDefinition->formalParaList.size()),
                                     currentSymbolTable);
...
void SemanticAnalyseVariant(_Variant *variant) {
    ...
    _SymbolRecord *record = findSymbolRecord(currentSymbolTable, variant->variantId.first,
                                             1);//用variantId.first去查符号表，检查是否重定义
    if (checkIsTheSameAsKey(variant->variantId.first, variant->variantId.second))//与库程序名、主程序名、主程序参数相同
        return;
    ...
    if (variant->type->flag == 0)//如果是普通变量
        ...
        currentSymbolTable->addArray(variant->variantId.first, variant->variantId.second, variant->type->type.first,
...
void SemanticAnalyseConst(_Constant *constant) {
    ...
    if (checkIsTheSameAsKey(constant->constId.first, constant->constId.second))//与库程序名、主程序名、主程序参数相同
        return;
    if (record != NULL) {//重定义 checked
        addDuplicateDefinitionErrorInformation(record->id, record->lineNumber, record->flag, record->type,
...
string SemanticAnalyseFunctionCall(_FunctionCall *functionCall) {
    ...
    if (record->flag != "function") {//不是函数 checked
        addPreFlagErrorInformation(functionCall->functionId.first, functionCall->functionId.second, "function",
                                   record->lineNumber, record->flag);
        return functionCall->returnType = "error";
    }
    ...
    if (functionCall->actualParaList.size() != record->amount) {//参数个数不一致 checked
        addNumberErrorInformation(functionCall->functionId.first, functionCall->functionId.second,
                                  int(functionCall->actualParaList.size()), record->amount, "function");
        return functionCall->returnType = record->type;
    }
    ...
    for (int i = 0; i < functionCall->actualParaList.size(); i++) {
        ...
        string formalType = record->findXthFormalParaType(i + 1);
        bool isRefered = record->isXthFormalParaRefered(i + 1);
        if (isRefered && !(functionCall->actualParaList[i]->type == "var" &&
                           (functionCall->actualParaList[i]->variantReference->kind == "var" ||
                            functionCall->actualParaList[i]->variantReference->kind == "array"))) {
            //该表达式不能作为引用形参对应的实参 checked
            addGeneralErrorInformation(
                    "[Referenced actual parameter error!] <Line " + itos(functionCall->actualParaList[i]->lineNumber) +
                    "> The " + itos(i + 1) +
                    "th actual parameter expression should be a normal variable、value parameter、referenced parameter or array element.");
            continue;
        }
        if (!isRefered) { //传值参数支持从integer到real的隐式类型转换
            if (actualType != formalType && !(actualType == "integer" && formalType == "real")) //checked
                addExpressionTypeErrorInformation(functionCall->actualParaList[i], actualType, formalType,
                                                  itos(i + 1) + "th actual parameter of function call of \"" +
                                                  functionCall->functionId.first + "\"");
        } else { //引用参数需保持类型强一致
...
string SemanticAnalyseExpression(_Expression *expression) {
    ...
    if (expression->type == "var") { //获得普通变量/常量/数组的类型 //如果是integer类型的常量
        ...
        if (expression->operationType == "relop") {
            ...
            if ((epType1 == epType2 && epType1 != "error") || (epType1 == "integer" && epType2 == "real") ||
                (epType1 == "real" && epType2 == "integer"))
        ...
        } else if (expression->operation == "+" || expression->operation == "-" || expression->operation == "*" ||
                   expression->operation == "/") {
            string epType1 = SemanticAnalyseExpression(expression->operand1);
            string epType2 = SemanticAnalyseExpression(expression->operand2);
            //checked
            if (expression->operation == "/" && epType2 == "integer" && expression->operand2->totalIntValueValid &&
                expression->operand2->totalIntValue == 0)
                addDivideZeroErrorInformation(expression->operation, expression->operand2);
            if (epType1 == "integer" && epType2 == "integer" && expression->operand1->totalIntValueValid &&
                expression->operand2->totalIntValueValid) {
                expression->totalIntValueValid = true;
                if (expression->operation == "+")
                    expression->totalIntValue =
                            expression->operand1->totalIntValue + expression->operand2->totalIntValue;
            ...
            string epType2 = SemanticAnalyseExpression(expression->operand2);
            //checked
            if (epType2 == "integer" && expression->operand2->totalIntValueValid &&
                expression->operand2->totalIntValue == 0)
                addDivideZeroErrorInformation(expression->operation, expression->operand2);
            if (epType1 == "integer" && epType2 == "integer") {
...
void SemanticAnalyseProgram(_Program *program) {
    ...
    lib.insert("writeln");
    lib.insert("exit");
    if (lib.count(program->programId.first)) //程序名不能和系统的库函数名相同，也就是不能用上面lib写的这几个东西
        addGeneralErrorInformation(
    ...
    //将主程序的参数(input,output)添加到主符号表中，flag定为"parameter of program"
    for (int i = 0; i < program->paraList.size(); i++) {//检查主程序参数是否和主程序名同名
        if (program->paraList[i].first == program->programId.first) //与主程序名同名
            addGeneralErrorInformation("[Duplicate defined error!] <Line " + itos(program->programId.second) +
                                       "> parameter of program \"" + program->programId.first +
                                       "\" is the same as name of program.");
        else if (lib.count(program->paraList[i].first)) //与库函数名同名
            addGeneralErrorInformation("[Dulicate defined error!] <Line " + itos(program->paraList[i].second) +
                                       "> parameter of program \"" + program->paraList[i].first +
                                       "\" has been defined as a lib program.");
        //如果没有错，那就随着循环一个一个加进去
    ...
    //amount=-1表示变参，即参数有多少个是都有可能的！(比如说read可以read(a,b,c)或者read(n)，你想写多少个参数都行，函数并没有定义固定要读入几个。
    //添加read过程，该过程变参
    ...
    //对主程序剩余部分（除了第一行程序名以外的部分）进行语义分析
    SemanticAnalyseRestProgram(program->subProgram);
}

string itos(int num) {
    stringstream ssin;
    ssin << num;  // 就是把int转成了string类型
    return ssin.str();
}
...
void addDuplicateDefinitionErrorInformation(string preId, int preLineNumber, string preFlag, string preType,
    ...
    if (preLineNumber != -1)
        errorInformation +=
                "\"" + preId + "\"" + " has already been defined as a " + preFlag + " at line " + itos(preLineNumber) +
                ".";
    else
...
void addUsageTypeErrorInformation(string curId, int curLineNumber, string curType, string usage, string correctType) {
    ...
    errorInformation +=
            "\"" + curId + "\"" + " used for " + usage + " should be " + correctType + " but not " + curType + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addNumberErrorInformation(string curId, int curLineNumber, int curNumber, int correctNumber, string usage) {
    ...
    if (usage == "array") {
        ...
        errorInformation +=
                "Array \"" + curId + "\"" + " should have " + itos(correctNumber) + " but not " + itos(curNumber) +
                " indices.";
    } else if (usage == "procedure") {
        errorInformation += "[Procedure parameter number mismatch!] ";
        errorInformation += "<Line " + itos(curLineNumber) + "> ";
        errorInformation +=
                "Procedure \"" + curId + "\"" + " should have " + itos(correctNumber) + " but not " + itos(curNumber) +
                " parameters.";
    } else if (usage == "function") {
        errorInformation += "[Function parameter number mismatch!] ";
        errorInformation += "<Line " + itos(curLineNumber) + "> ";
        errorInformation +=
                "Function \"" + curId + "\"" + " should have " + itos(correctNumber) + " but not " + itos(curNumber) +
                " parameters.";
    } else {
...
void addPreFlagErrorInformation(string curId, int curLineNumber, string curFlag, int preLineNumber, string preFlag) {
    ...
    errorInformation += "<Line " + itos(curLineNumber) + "> ";
    errorInformation +=
            "\"" + curId + "\"" + " defined at line " + itos(preLineNumber) + " is a " + preFlag + " but not a " +
            curFlag + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addExpressionTypeErrorInformation(_Expression *exp, string curType, string correctType, string description) {
    ...
    errorInformation +=
            "Expression \"" + expression + "\" used for " + description + " should be " + correctType + " but not " +
            curType + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addAssignTypeMismatchErrorInformation(_VariantReference *leftVariantReference, _Expression *rightExpression) {
    ...
    errorInformation += "<Left at line " + itos(leftVariantReference->variantId.second) + ", right at line " +
                        itos(rightExpression->lineNumber) + "> ";
    string varRef, exp;
    inputVariantRef(leftVariantReference, varRef, 1);
    inputExpression(rightExpression, exp, 1);
    errorInformation +=
            "Left \"" + varRef + "\" type is " + leftVariantReference->variantType + " while right \"" + exp +
            "\" type is " + rightExpression->expressionType + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addArrayRangeOutOfBoundErrorInformation(_Expression *expression, string arrayId, int X, pair<int, int> range) {
    ...
    errorInformation +=
            ", but the range of array \"" + arrayId + "\" " + itos(X) + "th index is " + itos(range.first) + " to " +
            itos(range.second) + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addArrayRangeUpSideDownErrorInformation(string curId, int curLineNumber, int X, int lowBound, int highBound) {
    ...
    errorInformation += "<Line " + itos(curLineNumber) + "> ";
    errorInformation +=
            itos(X) + "th range of array \"" + curId + "\" have larger low bound and smaller high bound, which is " +
            itos(lowBound) + " and " + itos(highBound) + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addOperandExpressionsTypeMismatchErrorInformation(_Expression *exp1, _Expression *exp2) {
    ...
    errorInformation += "Left \"" + expStr1 + "\" type is " + exp1->expressionType + " while right " + "\"" + expStr2 +
                        "\" type is " + exp2->expressionType + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addSingleOperandExpressionTypeMismatchErrorInformation(_Expression *exp, string correctType) {
    ...
    errorInformation +=
            "Expression \"" + expStr + "\" type should be " + correctType + " but not " + exp->expressionType + ".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addactualParameterOfReadErrorInformation(int curLineNumber, string procedureId, int X, _Expression *exp) {
    ...
    string expression;
    inputExpression(exp, expression, 1);
    errorInformation += "\"" + procedureId + "\" " + itos(X) + "th expression parameter \"" + expression +
                        "\" is not a variant or an array element.";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addDivideZeroErrorInformation(string operation, _Expression *exp) {
    ...
    inputExpression(exp, expression, 1);
    errorInformation +=
            "The value of expression \"" + expression + "\" is 0, which is the second operand of operation \"" +
            operation + "\".";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
...
void addReadBooleanErrorInformation(_Expression *exp, int X) {
    ...
    errorInformation += "<Line " + itos(exp->lineNumber) + "> ";
    errorInformation +=
            "The " + itos(X) + "th actual parameter of read \"" + expression + "\" is boolean, it can't be read.";
    semanticErrorInformation.push_back(errorInformation);
    CHECK_ERROR_BOUND
}
    ...
    for (int i = 0; i <= mainSymbolTable->recordList[0]->amount + 4; i++) {
        if (id == mainSymbolTable->recordList[i]->id) {
            if (i == 0) //与主程序名同名 checked
                addGeneralErrorInformation("[Duplicate defined error!] <Line " + itos(lineNumber) + "> \"" + id +
                                           "\" has been defined as the name of program at Line " +
                                           itos(mainSymbolTable->recordList[i]->lineNumber) + ".");
            else if (i >= 1 && i <= mainSymbolTable->recordList[0]->amount) //与主程序参数同名 checked
                addGeneralErrorInformation("[Duplicate defined error!] <Line " + itos(lineNumber) + "> \"" + id +
                                           "\" has been defined as a program parameter at Line " +
                                           itos(mainSymbolTable->recordList[i]->lineNumber) + ".");
            else //与库程序同名 checked
                addGeneralErrorInformation("[Duplicate defined error!] <Line " + itos(lineNumber) + "> \"" + id +
                                           "\" has been defined as a lib program.");
