...
//全局常量
vector<string> globalConstIdList;//常量标识符  
vector<string> globalConstTypeList;//常量类型,"integer","real","char","boolean"  
vector<string> globalConstValueList;//常量值,不论是哪种类型，均用string表示
//全局变量
vector<string> globalVariantIdList;//变量标识符  
vector<string> globalVariantTypeList;//变量类型,"integer","real","char","boolean"  
vector<vector<int> > globalArraySizeList;//各数组变量各维的大小

//子程序接口声明
struct subproDec {
    string returnType;//子程序返回值类型,"integer","real","char","boolean"
    string id;//子程序名称标识符
    vector<string> paraIdList;//参数标识符
    vector<bool> isParaRefList;//参数是否为引用
    vector<string> paraTypeList;//参数类型,"integer","real","char","boolean"
    subproDec() {
        paraIdList.clear();
        isParaRefList.clear();
        paraTypeList.clear();
...
vector<struct subproDec> subproDecList;//子程序接口列表
...
vector<pair<string, int> > globalStatementList;//主程序语句列表
...
void inputConstList(vector<_Constant *> &constList, vector<string> &constIdList, vector<string> &constTypeList,
                    vector<string> &constValueList, _SymbolTable *symbolTable);//获取常数列表
void inputVariantList(vector<_Variant *> variantList, vector<string> &variantIdList, vector<string> &variantTypeList,
                      vector<vector<int> > &arraySizeList, _SymbolTable *symbolTable);//获取变量列表
...
void inputVariantRef(_VariantReference *variantRefNode, string &variantRef, int mode = 0,
                     bool isReferedActualPara = false);//获取变量引用
void inputStatementList(_Statement *statementNode, vector<pair<string, int> > &statementList, int retract,
                        int flag = 0);//获取语句列表
...
bool checkAndInputLibrarySubprogram(_ProcedureCall *procedureCall, vector<pair<string, int> > &statementList,
                                    int retract); //检查并获取库程序
...
void codeGenerate(_Program *ASTRoot, string outName) {
        ...
        if ((record->flag == "procedure" || record->flag == "function") && record->subSymbolTable != NULL) {
            ...
            for (int i = 1; i <= record->amount; i++) {
                ...
                if (subTable->recordList[i]->flag == "var parameter")
                    dec.isParaRefList.push_back(true);
                else
    ...
    inputVariantList(ASTRoot->subProgram->variantList, globalVariantIdList, globalVariantTypeList, globalArraySizeList,
                     codeGenerateCurrentSymbolTable);//全局变量/主程序变量
    ...
    outputConstList(globalConstIdList, globalConstTypeList, globalConstValueList);//输出全局常量
    fout << endl;

    fout << "//Overall variable definition" << endl;
    outputVariantList(globalVariantIdList, globalVariantTypeList, globalArraySizeList);//输出全局变量
    fout << endl;
...
void inputConstList(vector<_Constant *> &constList, vector<string> &constIdList, vector<string> &constTypeList,
                    vector<string> &constValueList, _SymbolTable *symbolTable) {
    //初始化
    constIdList.clear();
    constTypeList.clear();
...
void inputVariantList(vector<_Variant *> variantList, vector<string> &variantIdList, vector<string> &variantTypeList,
                      vector<vector<int> > &arraySizeList, _SymbolTable *symbolTable) {
    //初始化
    variantIdList.clear();
...
void inputStatementList(_Statement *statementNode, vector<pair<string, int> > &statementList, int retract, int flag) {
    ...
    if (statementNode->type == "compound") {
        ...
        if (flag == 0)
            statementList.push_back(make_pair("{", retract - 1));
        for (int i = 0; i < compoundNode->statementList.size(); i++)
            inputStatementList(compoundNode->statementList[i], statementList, retract);
        if (flag == 0)
            statementList.push_back(make_pair("}", retract - 1));
    } else if (statementNode->type == "repeat") {
        _RepeatStatement *repeatStatementNode = reinterpret_cast<_RepeatStatement *>(statementNode);
        string condition;
        statementList.push_back(make_pair("do", retract));
        inputStatementList(repeatStatementNode->_do, statementList, retract + 1);
        inputExpression(repeatStatementNode->condition, condition);
        condition = "while(!(" + condition + "));";
        statementList.push_back(make_pair(condition, retract));
        ...
        condition = "while(" + condition + ")";
        statementList.push_back(make_pair(condition, retract));
        inputStatementList(whileStatementNode->_do, statementList, retract + 1);
        ...
        statementList.push_back(make_pair(
                "for(" + forStatementNode->id.first + " = " + start + "; " + forStatementNode->id.first + " <= " + end +
                "; " + forStatementNode->id.first + "++)", retract));
        inputStatementList(forStatementNode->_do, statementList, retract + 1);
        ...
        statementList.push_back(make_pair("if(" + condition + ")", retract));
        inputStatementList(ifStatementNode->then, statementList, retract + 1);
        if (ifStatementNode->els != NULL) {
            statementList.push_back(make_pair("else", retract));
            inputStatementList(ifStatementNode->els, statementList, retract + 1);
        }
        ...
        if (assignStatementNode->variantReference->kind == "function return reference") {//如果是返回值语句
            ...
            statementList.push_back(make_pair(variantRef, retract));
        } else {
            inputVariantRef(assignStatementNode->variantReference, variantRef);
            statementList.push_back(make_pair(variantRef + " = " + expression + ";", retract));
        }
        ...
        if (checkAndInputLibrarySubprogram(procedureCallNode, statementList, retract))
            return;
        ...
        procedureCall += ");";
        statementList.push_back(make_pair(procedureCall, retract));
    }
}
...
void inputSubproDef(_FunctionDefinition *functionDefinitionNode) {
    ...
    inputConstList(functionDefinitionNode->constList, tmp.constIdList, tmp.constTypeList, tmp.constValueList,
                   record->subSymbolTable);
    inputVariantList(functionDefinitionNode->variantList, tmp.variantIdList, tmp.variantTypeList, tmp.arraySizeList,
                     record->subSymbolTable);
...
void inputSubMainFunction(_Program *ASTRoot) {
    ...
    inputStatementList(ASTRoot->subProgram->compound, globalStatementList, 1, 1);
}
    ...
    if (id == "exit") {
        ...
        for (int i = 0; i < procedureCall->actualParaList.size(); i++) {
            ...
            if (typeFormat == "bool") {
                ...
                statementList.push_back(make_pair("if(" + expression + ")", retract + 1));
                statementList.push_back(make_pair("printf(\"true\");", retract + 2));
...
void outputSubproDec(subproDec &tmp, int flag) {//flag==0时表示接口声明,flag==1时表示定义时的子程序头
    ...
    fout << str << " " << tmp.id << "(";
    int m = int(tmp.paraIdList.size());
    for (int j = 0; j < m; j++) {
        if (j != 0)
            fout << ", ";
        fout << tmp.paraTypeList[j] << " ";
        if (tmp.isParaRefList[j])
            fout << "*";
        fout << tmp.paraIdList[j];
    }
    fout << ")";
    if (flag == 0)
        fout << ";";
...
void outputSubproDecList() {
    ...
    vector<struct subproDec> &vec = subproDecList;
    int n = int(vec.size());
    for (int i = 0; i < n; i++) {
        if (vec[i].id == "") {
...
void outputSubproDefList() {
    ...
    fout << "{" << endl;
    outputStatementList(globalStatementList);
    fout << "}" << endl << endl;
    for (int i = 0; i < n; i++) {
        outputSubproDec(subproDecList[i], 1);
        fout << "{" << endl;
